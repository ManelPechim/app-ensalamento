...
import { searchTurmaIdRepository } from "../repositories/turmas.repository.ts";
...
export const handleGetTurmaById = async (id: number) => {
  const turmaId = await searchTurmaIdRepository(id);
...
export const handleEditTurma = async (id: number, turmaBody: TurmaModel) => {
  const turmaId = await searchTurmaIdRepository(id);
  if (!turmaId) throw new AppError(`O id informado (${id}) da Turma para edição é inválido ou não existe`, Status.NotFound); // 404
...
export const handlePatchTurma = async (id: number, turmaBody: Partial<TurmaModel>) => {
  const turmaId = await searchTurmaIdRepository(id);
...
export const handleDeleteTurmaById = async (id: number) => {
  const turmaId = await searchTurmaIdRepository(id);
